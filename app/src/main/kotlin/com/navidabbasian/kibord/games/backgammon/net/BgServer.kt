...
/**
 * سرور میزبان تخته‌نرد: تنها مهمانِ بازی یک سوکت دارد. پیام‌ها خط‌به‌خط JSON هستند.
...
class BgServer(
    ...
    private fun handleClient(socket: Socket) {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val hello = decodeBgMessage(reader.readLine() ?: return@launch) as? BgMessage.Hello
                    ?: return@launch
                val error = withContext(Dispatchers.Main) { onClientJoin(hello.name) }
                writer.println(BgMessage.Welcome(ok = error == null, error = error ?: "").encode())
                if (error != null) {
                    socket.close()
                    return@launch
                }
    ...
    override fun broadcast(msg: BgMessage) {
