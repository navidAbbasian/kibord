...
/**
 * کلاینت مهمان: به سوکت میزبان وصل می‌شود، معرفی می‌کند و
 * پیام‌های وضعیت را روی نخ اصلی تحویل می‌دهد.
 */
class EfClient(
    private val scope: CoroutineScope,
    private val onMessage: (EfMessage) -> Unit,
    /** بعد از یک اتصال موفق، قطع شدن ارتباط با میزبان */
    private val onDisconnected: () -> Unit,
) : ClientLink<EfMessage> {
    ...
    fun connect(host: String, port: Int, name: String, onResult: (String?) -> Unit) {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                w.println(EfMessage.Hello(name).encode())
                val welcome = decodeEfMessage(reader.readLine() ?: "") as? EfMessage.Welcome
    ...
    override fun send(msg: EfMessage) {
        val w = writer ?: return
        scope.launch(Dispatchers.IO) {
            try {
