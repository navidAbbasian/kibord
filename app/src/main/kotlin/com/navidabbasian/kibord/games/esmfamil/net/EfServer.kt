...
/**
 * سرور میزبان: هر گوشی مهمان یک سوکت دارد. پیام‌ها خط‌به‌خط JSON هستند.
...
class EfServer(
    ...
    private fun handleClient(socket: Socket) {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val hello = decodeEfMessage(reader.readLine() ?: return@launch) as? EfMessage.Hello
                    ?: return@launch
                val error = withContext(Dispatchers.Main) { onClientJoin(hello.name) }
                writer.println(EfMessage.Welcome(ok = error == null, error = error ?: "").encode())
                if (error != null) {
                    socket.close()
                    return@launch
                }
                ...
                while (isActive) {
                    ...
                    val msg = decodeEfMessage(line) ?: continue
    ...
    override fun broadcast(msg: EfMessage) {
