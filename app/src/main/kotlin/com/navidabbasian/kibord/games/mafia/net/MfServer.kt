...
/**
 * سرور میزبان مافیا: هر گوشی مهمان یک سوکت دارد. پیام‌ها خط‌به‌خط JSON هستند.
...
class MfServer(
    ...
    private val onCommand: (playerName: String, msg: MfMessage) -> Unit,
    ...
    private fun handleClient(socket: Socket) {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val hello = decodeMfMessage(reader.readLine() ?: return@launch) as? MfMessage.Hello
                    ?: return@launch
                val error = withContext(Dispatchers.Main) { onClientJoin(hello.name) }
                writer.println(MfMessage.Welcome(ok = error == null, error = error ?: "").encode())
                if (error != null) {
                    socket.close()
                    return@launch
                }
