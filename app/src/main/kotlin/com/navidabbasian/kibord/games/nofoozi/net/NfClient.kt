...
/**
 * کلاینت مهمان نفوذی: به سوکت میزبان وصل می‌شود، معرفی می‌کند و
 * پیام‌های وضعیت را روی نخ اصلی تحویل می‌دهد.
 */
class NfClient(
    private val scope: CoroutineScope,
    private val onMessage: (NfMessage) -> Unit,
    /** بعد از یک اتصال موفق، قطع شدن ارتباط با میزبان */
    private val onDisconnected: () -> Unit,
) : ClientLink<NfMessage> {
    ...
    fun connect(host: String, port: Int, name: String, onResult: (String?) -> Unit) {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                w.println(NfMessage.Hello(name).encode())
                val welcome = decodeNfMessage(reader.readLine() ?: "") as? NfMessage.Welcome
    ...
    override fun send(msg: NfMessage) {
        val w = writer ?: return
        scope.launch(Dispatchers.IO) {
            try {
