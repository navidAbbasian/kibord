...
/**
 * سرور میزبان من کی‌ام: هر گوشی مهمان یک سوکت دارد. پیام‌ها خط‌به‌خط JSON هستند.
...
class WaServer(
    ...
    private fun handleClient(socket: Socket) {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val hello = decodeWaMessage(reader.readLine() ?: return@launch) as? WaMessage.Hello
                    ?: return@launch
                val error = withContext(Dispatchers.Main) { onClientJoin(hello.name) }
                writer.println(WaMessage.Welcome(ok = error == null, error = error ?: "").encode())
                if (error != null) {
                    socket.close()
                    return@launch
                }
    ...
    override fun broadcast(msg: WaMessage) {
